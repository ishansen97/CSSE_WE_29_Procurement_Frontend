...
export class ViewOrdersComponent implements OnInit {
  ...
  constructor(private purchaseOrderService: PurchaseOrderService, private supplierService: SupplierService, private itemService: ItemService) {
  ...
  checkOrderStatus(order: PurchaseOrderResponse) {
    if (order.purchaseOrder.ordStatus === 'Approved' || order.purchaseOrder.ordStatus === 'Rejected') {
      return true;
    }
    else {
      return false;
  ...
  sendForApproval(order: PurchaseOrderResponse) {
    this.sendOrderDetails = order.purchaseOrder;
